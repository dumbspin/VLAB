...
function setup() {
  ...
  changetint = color(252,89,196, 200);//Defining the net at starting
  aftercolour = color(255, 255, 255, darkness);//Defining the net colour after titration
...
function start() {
  ...
  normality_titrant = random(0.60, .70);
...
function nextpressed() {
  ...
  fetch('send.php', {
    ...
    body: JSON.stringify({ data1: normality_titrate, data2: volume_titrate, data3: vadded, data4: normality_titrant }),
  })
    .then(response => {
      if (response.ok) {
    ...
    .then(data => {
      ...
      if (data.message === 'Value received successfully (Page 2)') {
        ...
        window.location.href = 'StartHtml2.php';
      } else {
